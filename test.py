...
tf.app.flags.DEFINE_integer('random_split_mode', True, 'train_test_split mode')
    ...
    or config.working_dataset == 'UBC_interpolated' or config.working_dataset == 'UBC_hard':
      ...
      with tf.Session(config=session_config) as sess:
        ...
        for num_test in range(1, train_set + 1):
          ...
          for num_cam in range(1, cam_range + 1):
            ...
            for depth_file_name, label_file_name in zip(depth_file_list, label_file_list):
              ...
              if not os.path.exists(output_file_name):
                ...
                summary_str, inferred_image, pc_acc = sess.run([summary, rgba_image, pc_accuracy],
                                                                    feed_dict={images: cropped_depth_image,
                                                                               labels: cropped_label_image})

                cwa = np.concatenate([cwa, np.expand_dims(pc_acc, 0)], axis=0) if cwa.size else np.expand_dims(pc_acc, 0)
          ...
          if not config.working_dataset == 'MHAD_UBC':
            hdf5storage.write(np.transpose(cwa), path='/cwa', filename=sub_dir + 'class_wise_accuracy.mat',
        ...
        with tf.Session(config=session_config) as sess:
          if FLAGS.random_split_mode:
            ...
            test_logs, ckpt_dir, main_input_dir, output_sub_dir = config.get_directories(config.working_dataset)
            test_logs = os.path.join(test_logs, str(FLAGS.test_subject)) + '/'
            ckpt_dir = os.path.join(ckpt_dir, str(FLAGS.test_subject)) + '/'
            output_sub_dir = os.path.join(output_sub_dir, str(FLAGS.test_subject)) + '/'
            ...
            for sub in range(1, subjects + 1):
              for act in range(1, actions + 1):
                ...
                for rec in range(1, recordings + 1):
                  ...
                  for depth_file_name, label_file_name in zip(depth_file_list, label_file_list):
                    ...
                    if not os.path.exists(output_file_name):
                      ...
                      try:
                        ...
                        cropped_label_image = label_image_array[min(itemidy[0]):max(itemidy[0]),
                                        min(itemidx[0]):max(itemidx[0]), :]
                        resized_label_image = cv.resize(cropped_label_image, (224, 224), interpolation=cv.INTER_LINEAR)

                      # Infer body-part labels from the learned model
                        summary_str, inferred_image, pc_acc = sess.run([summary, rgb_image, pc_accuracy],
                                                             feed_dict={images: resized_depth_image,
                                                                        labels: resized_label_image })

                        cwa = np.concatenate([cwa, np.expand_dims(pc_acc, 0)], axis=0) if cwa.size else np.expand_dims(
                          pc_acc, 0)
                  ...
                  sub_dir = os.path.join(main_input_dir, output_sub_dir,
                                         'Kin%02d/S%02d/A%02d/R%02d_cwa/' % (cam, sub, act, rec))
      ...
      with tf.Session(config=session_config) as sess:
        ...
        for num_test in range(1, train_set + 1):
          cwa = np.array([])
          for num_cam in range(1, cam_range + 1):
            print 'Currently processing subject number ' + str(num_test) + ' camera no- ' + str(num_cam)
            ...
            for depth_file_name in depth_file_list:
              ...
              if not os.path.exists(output_file_name):
                ...
                # Find Min-max non zero pixels in the label image
                depth_image_array = np.array(depth_image).sum(axis=2)
